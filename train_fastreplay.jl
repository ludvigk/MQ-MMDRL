...
using MLUtils
using ADTypes
using NNlib                     # for relu broadcasting etc.
...
- ys: (N, na, B)
- actions_in: Vector{Int} or CuArray{Int}
Returns (N,B)
"""
...
# ========= Training =========
# NOTE: We assume `atari_env` is available from your env module (UInt8 stacked obs).
# If it's in another file, `include("your_atari_env.jl")` above.
function train(game::String="breakout"; lambda::Float32=0.5f0)
    ...
    # Envs (must return UInt8 stacked states)
    ...
    # Net: Avoid fused conv+act (separate Conv and ReLU). Safer for AD + cuDNN.
    net = Lux.Chain(
    ...
    # Robust backend (avoids Enzyme×cuDNN crash)
    backend = AutoZygote()
    # backend = AutoEnzyme()

    while t < T
        CRL.reset!(env)
        frame = CRL.observe(env)                                # UInt8 (84,84,4)
        @views FastReplay.push!(rb, frame[:,:,end], 0, 0f0, false)  # seed
        ...
        while !CRL.terminated(env)
            ...
            action = if rand() < eps || t < WARMUP_FRAMES
                rand(CRL.actions(env))
            else
                sdev = reshape(frame, 84,84,4,1) .|> Float32 |> todev
                sdev ./= 255f0
            ...
            total_reward += reward
            frame = CRL.observe(env)                            # UInt8 (84,84,4)
            done = CRL.terminated(env)

            @views FastReplay.push!(rb, frame[:,:,end], action, reward, done)

            if rb.size >= max(WARMUP_FRAMES, 4B)
                for _ in 1:UPDATES_PER_STEP
                    batch = FastReplay.sample_batch(rb, B)
                    # Already packed by FastReplay
                    s_u8   = batch.states         # (84,84,4,B)
                    sp_u8  = batch.next_states    # (84,84,4,B), zeros for terminals
                    next_m = batch.next_mask      # BitVector(B)
                    # ---------- Target ----------
                    bs  = todev(s_u8  .* (1f0/255))
                    bsp = todev(sp_u8 .* (1f0/255))

                    ys1, st = Lux.apply(ts.model, bsp, target_ps, ts.states)
                    ...
                    r        = todev(Float32.(batch.rewards))   # (B,)
                    target_ys = γ .* (ys1_sel .* reshape(not_done,1,B)) .+ reshape(r,1,B)

                    # ---------- Train ----------
                    data = (bs, target_ys, batch.actions, cum_prob, lambda)
            ...
            if t % 250_000 == 0
                ...
                while t_eval < 125_000
                    ...
                    while !CRL.terminated(eval_env)
                        t_eval += 1
                        sdev = reshape(CRL.observe(eval_env), 84,84,4,1) .|> Float32 |> todev
                        q, st = q_val(sdev, ts.model, ts.parameters, ts.states)
                        ai = argmax(q)
...
function main()
    ...
    # NOTE: assumes `atari_env` is defined in your environment module already loaded
